...
def NaturalSelection(AllWeights,Ancestors,Scores,NetSize,mutation,step,Nnets,pAddNeuron):
    #StepSize=.3
    #StepSizeAdd=1
    #StepSizem1=.3
    Padd=.1
    ...
    NewAncestors=[]
    Ps=[]
    VeryBest=np.percentile(Scores,10)
    for i in range(9):
    ...
    for i in range(int(np.floor(Nnets/10))): #Make 10 completely new NNs
        Weights=[]
        NN=Neural_Network2(deepcopy(NetSize),Weights)
...
def CopyWeights(Ws):
    Weights=[]
    for W in Ws:
        Weights.append(torch.FloatTensor(W))
    return Weights